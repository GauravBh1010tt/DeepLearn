...
def load_sick(model_name, wordVec=None):
    ...
    data = pd.read_csv('data/train_features.csv').values
    ques_sent,ans_sent,q_test,a_test = [],[],[],[]
    for i in data:
        ques_sent.append(i[0].split())
        ans_sent.append(i[1].split())
        
    data = pd.read_csv('data/test_features.csv').values
    for i in data:
        q_test.append(i[0].split())
        a_test.append(i[1].split())
    #data=[],[],[]
    
    score = np.array(pd.read_pickle("data/train_labels.pkl")).tolist()
    score = [i[0]for i in score]
    
    sc_test = np.array(pd.read_pickle("data/test_labels.pkl")).tolist()
